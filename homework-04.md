...
Предположу что более высокая скокость выполнения запроса с использованием aggregation pipeline (среднее около 2700ms) по сравнению с map reduce (среднее около 5300ms) обусловлено тем что map reduce помимо получения результатов еще и сохраняет их в коллекцию.
